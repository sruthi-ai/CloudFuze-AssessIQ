 ...
 *   50-question content-migration pool: 15 process (sequence/dependency),
...
export const CONTENT_MIGRATION_QUESTIONS: { body: string; options: string[]; correct: number; category: Category }[] = [
  ...
  { category: 'process', body: "A link inside a moved document still points to the OLD file, not the new one. Is that always a mistake?", options: ["Yes — this always means the whole migration crashed", "Not always — the link can only be fixed once the file it points to has also finished moving", "No — links are never meant to be fixed automatically", "It only updates once the client manually asks for it"], correct: 1 },
  ...
  { category: 'process', body: "A check for \"new changes\" comes back with nothing for a folder. Does that mean nothing changed anywhere inside it?", options: ["Yes — \"no changes\" always means the whole folder tree is confirmed clean", "It only checks files changed in the last 24 hours", "Not necessarily — it may still need to check the folders inside that folder", "Inner folders are checked on a separate weekly schedule"], correct: 2 },
  ...
  { category: 'process', body: "Why does fixing a broken link have to wait until AFTER the file with the link has already moved?", options: ["Links and files always move in the exact same step", "You need the moved copy of the file to exist so you can open it and fix what's inside", "Links are fixed before the move to save time later", "This is only true for large files"], correct: 1 },
  ...
  { category: 'process', body: "Why would moving files from Dropbox work differently than moving from a system that tracks each file by a permanent ID?", options: ["Dropbox tracks files mainly by name/location, so a rename can look like a totally different file", "Dropbox doesn't support any kind of update-tracking at all", "ID-based systems are always faster, no matter what", "This only affects who can access the file, not the file itself"], correct: 0 },
  ...
  { category: 'process', body: "A client wants to skip a broken folder and move its contents straight to the top level instead. What should you think about first?", options: ["This is always the right fix for any broken folder", "This changes the final folder structure — check if that's really what they want, or if they just want the folder unstuck", "This request is impossible and must be refused", "This only matters for folders with a lot of files in them"], correct: 1 },
  ...
  { category: 'technical', body: "How would the system likely decide which failures to retry automatically vs. send to a person to check?", options: ["Every failure gets retried the same fixed number of times", "It probably sorts failures by type, and only auto-retries the ones known to usually fix themselves", "A person is only involved after a client complains", "Which failures retry is picked at random"], correct: 1 },
  { category: 'technical', body: "How might the system avoid creating the same folder twice if a retry happens?", options: ["It relies on a person to delete duplicates afterward", "It always deletes and recreates the folder from scratch every retry", "It likely checks if a matching folder already exists at the destination before creating a new one", "Duplicate folders are prevented only by the cloud provider's own naming rules"], correct: 2 },
  { category: 'technical', body: "Why would checking for new changes work differently for a system that tracks files by file path, versus one that tracks by a permanent ID?", options: ["Both types use the exact same method underneath", "An ID-based system can check a feed of changes by ID; a path-based one has to compare current state to what it looked like before", "Path-based systems simply cannot check for changes at all", "ID-based systems require the client to manually confirm every change"], correct: 1 },
  { category: 'technical', body: "Why would fixing links need to be a separate pass over files that already moved, instead of happening while they move?", options: ["Fixing links only matters for legal record-keeping", "Links are fixed before the move to save time", "Fixing a link means knowing where its target ended up — which might not exist yet at the exact moment the file itself is moving", "This is just an old design choice with no real reason behind it"], correct: 2 },
  ...
  { category: 'technical', body: "Why does the system keep a file's original date instead of stamping it with today's date when it moves?", options: ["Dates are always changed to the move date for record-keeping", "Keeping dates only works between two accounts on the same platform", "Dates don't actually matter and are never tracked either way", "Keeping the real date preserves an accurate history of the file for the client — using today's date would lose that"], correct: 3 },
  ...
  { category: 'technical', body: "Why might moving one giant file fail more easily than moving many small files that add up to the same total size?", options: ["Big files are always damaged during compression before the move", "Small files are somehow immune to network problems", "File size has nothing to do with how often something fails", "One big file spends more time exposed to a network hiccup in a single attempt; small files fail and retry quickly, one at a time"], correct: 3 },
  ...
  { category: 'technical', body: "Why would retrying a failed FOLDER need an extra \"does this already exist?\" check that retrying a failed FILE doesn't need as much?", options: ["Folders are retried far less often, so this is just an extra precaution", "This check is a legal requirement specific to folders, not files", "Files are never retried, so the comparison doesn't apply to them", "A folder retry might be re-attempting something that partly succeeded before — checking first avoids creating a duplicate or orphaned folder"], correct: 3 },
  ...
  { category: 'flow', body: "A client says a link still points to the old file weeks after the migration was reported \"complete.\" What do you check?", options: ["Whether the client's browser cache needs clearing", "Whether someone edited the link by hand afterward", "Whether the file the link points to actually finished moving — if not, the link couldn't have been fixed yet", "Whether the link uses http instead of https"], correct: 2 },
  ...
  { category: 'flow', body: "A client on Dropbox says files they renamed aren't showing the new name at the destination. What's likely going on?", options: ["Assume Dropbox doesn't support renaming at all", "Dropbox may be reading the rename as \"old file gone, new file appeared\" instead of an actual rename", "Assume the destination is blocking renamed files on purpose", "Assume it just needs 24 more hours with no other checking"], correct: 1 },
  ...
  { category: 'flow', body: "A client's calendar-style meeting data (if part of their migration) shows the wrong time after moving. What do you check first?", options: ["How time zones are being handled — this is a known trouble spot for date/time data specifically", "Whether the client's computer clock is wrong", "Assume the client edited the event afterward", "Assume this is a display issue, unrelated to the move"], correct: 0 },
  { category: 'flow', body: "A client's folder structure looks reorganized at the destination, not matching the original. What's your first question?", options: ["Assume the destination can't support the same folder structure at all", "Whether folder matching was set up to keep the same structure, or whether some folders didn't have a direct match and got placed differently", "Assume this always means the migration is broken and needs a full restart", "Assume the client reorganized it themselves afterward"], correct: 1 },
  { category: 'flow', body: "A client looking at an old saved report link says it looks different from what your team sees. What do you check?", options: ["Assume their browser needs updating", "Assume the link expired and start a whole new job", "Whether they're looking at an older version of the report page while your team sees the newer one", "Assume the underlying data is actually different for them"], correct: 2 },
  { category: 'flow', body: "A client's folder shows every file inside as \"done,\" but the folder itself still says \"in progress.\" What do you check?", options: ["Assume this combination is impossible and must be a bug", "Assume the top-level status is always a day behind", "Whether there's a hidden folder not shown in the current view, or a final check still running after the visible files finished", "Assume the report is just cached and stale"], correct: 2 },
  { category: 'flow', body: "A client's migration shows 8,000 of 65,000 files done, with several folders already fully finished. Is this a problem?", options: ["Yes — anything under half-done should be escalated right away", "Not necessarily — check whether the remaining folders are just big; only worry if progress has genuinely stopped", "Assume the count is wrong and recalculate it by hand first", "Assume the smaller, finished folders are secretly failing"], correct: 1 },
  { category: 'flow', body: "A shared folder's access list at the new location doesn't match who had access at the old one. What do you check first?", options: ["Assume the destination has a hard limit on how many people can have access", "Assume access was lost due to a random network issue", "Whether everyone from the old list has a matching account at the new location — anyone missing that match is a real gap", "Assume the source never tracked access correctly to begin with"], correct: 2 },
...
async function main() {
  ...
  if (!bank) {
    bank = await prisma.questionBank.create({ data: { name: CONTENT_MIGRATION_BANK_NAME, tenantId, description: 'Content migration engineering reasoning MCQ pool — 15 process, 15 technical, 20 flow-driven troubleshooting questions.' } })
  }

...
export async function sessionRoutes(server: FastifyInstance) {
  ...
  // wifi), and hundreds of them may log in within the same few minutes. The keyspace (8 chars
  // from a 32-char alphabet, ~10^12 combinations) makes brute force infeasible even at 500/5min.
  server.get('/by-pin/:pin', { config: { rateLimit: { max: 500, timeWindow: '5 minutes' } } }, async (request, reply) => {
